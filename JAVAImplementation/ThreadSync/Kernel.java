/* ---------------------------------------------------------------
  ...
  Task: Modify the code of the WAIT and EXIST case in Kernel.java
  		using SyncQueue.java, so that threads can wait for one of
		their child threads to be terminated. Note that SyncQueue.java
		should be instantiated as waitQueue upon a boot, (i.e., in the BOOT case).
			...
			case INTERRUPT_SOFTWARE: // System calls
	    		switch( cmd ) { 
	    			...
	    			case WAIT:
						...
						if (myTcb != null) {
							int tid = myTcb.getTid();		//get the current thread id
							return waitQueue.enqueueAndSleep(tid);		//let the current thread sleep in waitQueue under the condition = this thread id
						}
					...
					case EXIT:
						...
						if (myTcb != null) {
							int tid = myTcb.getPid();		//get the current thread's parent id
							waitQueue.dequeueAndWakeup(tid, myTcb.getTid());		// search waitQueue for and wakes up the thread under the condition = the current thread's parent id
							scheduler.deleteThread();		// tell the Scheduler to delete the current thread (since it is exiting)
							return OK;
						}
	    			...
	    			case RAWREAD: // read a block of data from disk
						while ( disk.read( param, ( byte[] )args ) == false ); // busy wait
						while ( disk.testAndResetReady( ) == false ); // busy wait
						return OK;
	    			case RAWWRITE: // write a block of data to disk
						while ( disk.write( param, ( byte[] )args ) == false ); // busy wait
						while ( disk.testAndResetReady( ) == false ); // busy wait
						return OK;
